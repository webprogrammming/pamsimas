...
class RiwayatPembayaranControllerTest extends TestCase
{
    ...
    {
        $response = $this->get('/riwayat-pembayaran');
        $response->assertStatus(200); // Check if status is 200
        $response->assertViewIs('riwayat-pembayaran.index'); // Check if the correct view is used
    }
    ...
    public function testGetDataWithDateRange()
    {
        ...
        $response = $this->withoutMiddleware()->get('/riwayat-pembayaran/get-data', [
    ...
    public function testPrintPdf()
    {
        ...
        // Call the route to print the payment history
        $response = $this->get("/riwayat-pembayaran/print/{$pembayaran->id}");
